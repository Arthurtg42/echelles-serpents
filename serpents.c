...
** InsertTri
**
** Insère une case de numéro val "au bon endroit" dans la liste chaînée grille (ordonnéé)
** et met à jour les pointeurs "suivante"
...
** CaseMax
**
** Renvoi le plus grand numéro de la liste chaînée
** 0 si grille est NULL
** grille une pointeur sur une liste chaînée ordonnée
*/
...
** Si le déplacement "sort" de la grille renvoi NULL (il faudra donc se replacer à la première case)
*/
...
** Créer cette case piege si elle n'existe pas.
** La liste chaînée garde son ordre croissant.
** Les pointeurs suivante et piege pour chaque case sont initilisés
*/
...
**
** Parcours la liste chaînée et initialise tous les pointeurs vers les cases précédentes
*/
void InitPointeurPrecedente(CaseJeu** grille){
	CaseJeu* precedent = *grille;
	CaseJeu* courant = (*grille)->suivante;
	// Boucle pour initialiser les pointeurs vers les cases précédentes
	while(courant != NULL){
...
CaseJeu* lire_fichier_grille(char* nom_fichier) {
	...
	if(grille != NULL && grille->suivante != NULL){
		...
		printf("Comme nous lançons 2 dés à 6 faces, il nous faut au moins 2 cases pour jouer !\n");
	}
	return grille;
}
...
void jouer_jeu(CaseJeu* grille) {
	...
	//C.I : saisie != '0' (le joueur ne veut pas arretez)
		...
		// sinon, Deplace renvoi NULL il faut se repositionner au début
			...
			printf("Lancé de dés trop élevé : ");
			pos = grille;
		}
...
int main(/*int argc, const char * argv[]*/) {
	...
	// Si la grille contient strictement moins de 2 cases on ne lance pas le jeu
	if(grille == NULL || (grille != NULL && grille->suivante == NULL))
		return 0;
	printf("La grille de jeu : \n(case precedente) <- case -> (case suivante) | piege potentiel\n");
	Affiche(grille);
	jouer_jeu(grille);
